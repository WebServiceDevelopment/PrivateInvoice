...
const ActionWidget = function () {
    ...
    this.DOM = {
        ...
        invoice: {
            ...
            sendInvoiceDisabled: () => {
                this.DOM.invoice.sendInvoice.setAttribute(
                    'disabled',
                    'disabled'
                )
        ...
        viewReport: {
            // paid
            moveToArciveBuyer: Elem(
                'ActionWidget.viewReport.moveToArciveBuyer'
            ),
            moveToArciveSeller: Elem(
                'ActionWidget.viewReport.moveToArciveSeller'
            ),
    ...
    this.EVT = {
        ...
        // modal_transaction
        handleModalTransactionCloseClick:
            evt_handleModalTransactionCloseClick.bind(this),

        // modal_receipt
        handleModalReceiptCloseClick:
            evt_handleModalReceiptCloseClick.bind(this),

        handleBuyerWaitingClick: evt_handleBuyerWaitingClick.bind(this),
        handleMakePaymentWaitingClick:
            evt_handleMakePaymentWaitingClick.bind(this),
        ...
        handleViewReport: {
            moveToArciveBuyerClick:
                evt_handleViewReport_MoveToArciveBuyerClick.bind(this),
            moveToArciveSellerClick:
                evt_handleViewReport_MoveToArciveSellerClick.bind(this),

            completeBuyerClick:
                evt_handleViewReport_CompleteBuyerClick.bind(this),
            completeSellerClick:
                evt_handleViewReport_CompleteSellerClick.bind(this),
        },
        ...
        handleIpfs: {
            ...
            jsonDataByTransactionClick:
                evt_handleIpfs_jsonDataByTransactionClick.bind(this),
        },
    }
    ...
    this.SENDING_WRAP = {
        visible: () => {
            this.DOM.invoice.sending.style.visibility = 'visible'
        },
        hidden: () => {
            this.DOM.invoice.sending.style.visibility = 'hidden'
        },
    }
    ...
    function init() {
        this.DOM.invoice.removeDraft.addEventListener(
            'click',
            this.EVT.handleInvoiceDraftDelete
        )
        this.DOM.invoice.sendInvoice.addEventListener(
            'click',
            this.EVT.handleInvoiceDraftSend
        )
        this.DOM.invoice.returnInvoice.addEventListener(
            'click',
            this.EVT.handleInvoiceReturnClick
        )
        this.DOM.invoice.confirmInvoice.addEventListener(
            'click',
            this.EVT.handleInvoiceConfirmClick
        )
        this.DOM.invoice.moveToTrash.addEventListener(
            'click',
            this.EVT.handleInvoiceTrashClick
        )
        this.DOM.invoice.recreate.addEventListener(
            'click',
            this.EVT.handleInvoiceRecreate
        )
        this.DOM.invoice.unconfirm.addEventListener(
            'click',
            this.EVT.handleInvoiceUnconfirm
        )
        this.DOM.invoice.makePayment.addEventListener(
            'click',
            this.EVT.handleInvoiceMakePayment
        )
        this.DOM.invoice.withdrawSent.addEventListener(
            'click',
            this.EVT.handleInvoiceWithdrawSent
        )

        // modal
        this.DOM.modal.close.addEventListener(
            'click',
            this.EVT.handleModalCloseClick
        )
        this.DOM.modal.cancel.addEventListener(
            'click',
            this.EVT.handleModalCloseClick
        )
        this.DOM.modal.submit.addEventListener(
            'click',
            this.EVT.handleModalSubmitClick
        )

        //  modal_transaction
        this.DOM.modal_transaction.close.addEventListener(
            'click',
            this.EVT.handleModalTransactionCloseClick
        )
        this.DOM.modal_transaction.cancel.addEventListener(
            'click',
            this.EVT.handleModalTransactionCloseClick
        )

        //  modal_receipt
        this.DOM.modal_receipt.close.addEventListener(
            'click',
            this.EVT.handleModalReceiptCloseClick
        )
        this.DOM.modal_receipt.cancel.addEventListener(
            'click',
            this.EVT.handleModalReceiptCloseClick
        )
        ...
        this.DOM.invoice.makePaymentWaiting.addEventListener(
            'click',
            this.EVT.handleMakePaymentWaitingClick
        )

        // paid
        // [ View Taransaction Receipt ]
        this.DOM.viewReport.moveToArciveBuyer.addEventListener(
            'click',
            this.EVT.handleViewReport.moveToArciveBuyerClick
        )

        // [ View Taransaction Receipt ]
        this.DOM.viewReport.moveToArciveSeller.addEventListener(
            'click',
            this.EVT.handleViewReport.moveToArciveSellerClick
        )

        // complete
        // [ View Taransaction Receipt ]
        this.DOM.viewReport.completeBuyer.addEventListener(
            'click',
            this.EVT.handleViewReport.completeBuyerClick
        )

        // [ View Taransaction Receipt ]
        this.DOM.viewReport.completeSeller.addEventListener(
            'click',
            this.EVT.handleViewReport.completeSellerClick
        )

        //

        this.DOM.invoice.sellerArchive.addEventListener(
            'click',
            this.EVT.handleSupplyArchiveClick
        )
        this.DOM.invoice.buyerTrash.addEventListener(
            'click',
            this.EVT.handleBuyerTrashClick
        )
        this.DOM.invoice.sellerTrash.addEventListener(
            'click',
            this.EVT.handleSupplyTrashClick
        )

        this.DOM.modal.gasLimit_input.addEventListener(
            'blur',
            this.EVT.handleGasLimitBlur
        )

        this.DOM.modal_transaction.ipfs_cid.addEventListener(
            'click',
            this.EVT.handleIpfs.jsonDataClick
        )

        this.DOM.modal_receipt.ipfs_cid.addEventListener(
            'click',
            this.EVT.handleIpfs.jsonDataByTransactionClick
        )
    ...
    async function evt_handleInvoiceMakePayment() {
        ...
        let ids = ['balanceGwei', 'makePaymantTo', 'transferCost', 'gas']
        ...
        let balance = parseInt(
            msg[ids[0]].replace(/,/g, '').replace('Gwei', '')
        )
        let amount = 0
    ...
    async function api_handleModalSubmit() {
        const url = '/api/invoice/?action=pay'
        const doc = DocumentWidget.API.getDocument()
    ...
    async function evt_handleInvoiceConfirmClick() {
        ...
        const url = '/api/invoice/?action=confirm'
    ...
    async function network_invoiceDraftSend() {
        ...
        const url = '/api/invoice/?action=send'
