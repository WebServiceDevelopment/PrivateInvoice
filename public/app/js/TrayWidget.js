...
const TrayWidget = function () {
    ...
    function init() {
        this.DOM.header.select.addEventListener(
            'change',
    ...
    function evt_handleBodyClick(evt) {
        ...
        DocumentWidget.API.openDocument(
            elem.memberData.doc.document_uuid,
            role,
            folder,
            archive
        )
    ...
    function api_renderFolder(role, folder, archive) {
        ...
        const session = SessionWidget.API.getSessionMemory()
        const results = this.MEM.getResults()
        let count = 0
        ...
        results.forEach((doc) => {
            ...
            cell_a1.title =
                'last action : ' + doc.seller_last_action.substr(0, 16)
        ...
        if (folder == 'trash' || archive == 1) {
            ...
            FolderWidget.API.getInvoiceArchiveCount(
                count,
                role,
                folder,
                archive,
