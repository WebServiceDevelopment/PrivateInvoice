...
router.post('/tellMeYourWalletAccount', async function (req, res) {
    ...
    // 1.
    //
    const [seller_host, err1] = await sub.getBuyerHost(
        CONTACTS,
        seller_did,
        buyer_did
    )
...
router.post('/sellerToConnect', async (req, res) => {
    ...
    // 1.
    //
    const [seller_host, err1] = await sub.getSellerHost(
        CONTACTS,
        seller_uuid,
        buyer_did
    )
...
router.post('/sellerToConfirm', async (req, res) => {
    ...
    const [_, err] = await sub.setConfirm(
        SELLER_STATUS,
        document_uuid,
        buyer_did
    )
...
router.post('/sellerToUnconfirm', async (req, res) => {
    ...
    const [_, err] = await sub.setUnconfirm(
        SELLER_STATUS,
        document_uuid,
        buyer_did
    )
...
router.post('/sellerToReturn', async (req, res) => {
    ...
    const [_, err] = await sub.setReturn(
        SELLER_STATUS,
        document_uuid,
        buyer_did
    )
...
router.post('/sellerToMakePayment', async (req, res) => {
    ...
    console.log(
        '/sellerToMakePayment : totalPaymentDue=' + totalPaymentDue.price
    )
    ...
    // 6.
    //
    const [_6, err6] = await tran.setMakePayment_status(
        conn,
        SELLER_STATUS,
        document_uuid,
        buyer_did
    ...
    // 7.
    //
    const [_7, err7] = await tran.setMakePayment_document(
        conn,
        SELLER_DOCUMENT,
        document_uuid,
        hash
...
router.post('/sellerToPaymentReservation', async (req, res) => {
    ...
    const [_, err] = await sub.setPaymentReservation(
        SELLER_STATUS,
        document_uuid,
        buyer_did
    )
...
router.post('/sellerToCancelPaymentReservation', async (req, res) => {
    ...
    const [_, err] = await sub.resetPaymentReservation(
        SELLER_STATUS,
        document_uuid,
        buyer_did
    )
...
router.post('/sellerRollbackReturnToSent', async (req, res) => {
    ...
    const [_, err] = await sub.rollbackReturnToSent(
        SELLER_STATUS,
        document_uuid,
        buyer_did
    )
...
router.post('/sellerRollbackConfirmToSent', async (req, res) => {
    ...
    const [_, err] = await sub.rollbackConfirmToSent(
        SELLER_STATUS,
        document_uuid,
        buyer_did
    )
...
router.post('/sellerRollbackSentToConfirm', async (req, res) => {
    ...
    const [_, err] = await sub.rollbackSentToConfirm(
        SELLER_STATUS,
        document_uuid,
        buyer_did
    )
...
router.post('/sellerRollbackPaidToConfirm', async (req, res) => {
    ...
    const [_, err] = await sub.rollbackPaidToConfirm(
        SELLER_STATUS,
        document_uuid,
        buyer_did
    )
